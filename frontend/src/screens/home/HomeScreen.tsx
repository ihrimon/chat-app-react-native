...
export default function HomeScreen() {
    ...
    Alert.alert(
      ...
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            await logout();
            navigation.replace('AuthNavigator');   // AuthNavigator-এ ফিরে যাবে
          },
        },
      ]
    );
  ...
  // Status Data
  const statusData = [
    { id: 'my', name: 'My status', image: user?.avatar || 'https://i.pravatar.cc/300', isOwn: true },
    ...chats.slice(0, 6).map((chat: any) => ({
      id: chat._id,
      name: chat.otherUser?.name?.split(' ')[0] || 'User',
      image: chat.otherUser?.avatar || 'https://i.pravatar.cc/300',
    })),
  ];
  ...
  const renderChatItem = ({ item }: any) => {
    ...
    return (
      ...
      >
        <Image
          source={{ uri: otherUser.avatar || 'https://i.pravatar.cc/300' }}
  ...
  return (
    <View className="flex-1 bg-[#0F0F0F]">
      ...
      <View className="flex-row items-center justify-between px-4 pt-12 pb-4 bg-[#0F0F0F]">
        <Text className="text-white text-2xl font-bold tracking-tight">Talkify</Text>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View className="flex-1 justify-center items-center mt-20">
              ...
              <Text className="text-gray-400 text-lg mt-6">No conversations yet</Text>
      ...
      <View className="absolute bottom-0 left-0 right-0 bg-[#1A1A1A] border-t border-gray-800">
        <View className="flex-row justify-around items-center py-3">
          <TouchableOpacity className="items-center">
            ...
            <Text className="text-[#24786D] text-xs mt-1 font-medium">Message</Text>
          </TouchableOpacity>
          ...
          <TouchableOpacity className="items-center" onPress={handleLogout}>
            ...
            <Text className="text-gray-400 text-xs mt-1">Logout</Text>
          </TouchableOpacity>
        </View>
      </View>
    </View>
